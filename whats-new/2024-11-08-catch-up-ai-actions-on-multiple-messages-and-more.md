---
title: Catch-up, AI Actions on multiple messages, and more
description: Summarize a busy conversation with Catch-up, run AI Actions across several messages at once, raise your phone to your ear to listen, and name your voice memos.
date: 2024-11-08
tags: [release, ai, playback, voice-memos, conversations]
---

# Catch-up, AI Actions on multiple messages, and more

This release is about catching up quickly and turning discussions into structured
updates.

<!-- truncate -->

## Quickly catch up on missed messages

Stuck in a meeting, or waking up to a busy conversation? Swipe right to left on the
conversation from Home and **Catch-up** summarizes the discussion, with links to play the
source messages. You can mark everything read from there too.

## A history of your AI Actions

Tap the AI icon in the bottom right of a conversation to find every AI Action result from
across that conversation in one place.

## AI Actions across multiple messages

Want to turn a whole discussion or several voice memos into a presentation outline or a
meeting summary with action items? Select the messages you want included and run the
action on all of them at once.

1. Press and hold any voice memo or message in a conversation.
2. Tap **Select** to choose multiple messages.
3. Pick your AI Action.

## Raise your phone to your ear to play new messages

To listen privately without headphones, bring your phone to your ear and unheard messages
in the conversation start playing. Playback pauses when you move the phone away and
resumes when you bring it back. Turn it on in **Profile Menu → Settings → Recording &
Playback**.

## Naming your voice memos

Struggling to remember which memo is which? AI now names them automatically, and you can
rename them yourself:

1. Open your list of voice memos.
2. Each memo has an auto-generated title — very short memos won't.
3. Tap the title to edit it.

## Convert a DM into a named conversation

You asked for this: a direct message can become a named conversation, so you can add and
remove members, preserve the history for people who join later, and get more access
control options.

1. In the DM, tap **⋯** in the top-right for **Conversation Settings**.
2. Tap **Members**.
3. Tap **Convert DM into Named Conversation**.
4. Name it and tap **Convert**.

## Start a new conversation with the same members

New topic, same group of people? You no longer have to rebuild the member list.

1. In the conversation, tap **⋯** in the top-right for **Conversation Settings**.
2. Tap **Members**.
3. Tap **⋯** in the top-right.
4. Select **New Conversation +**.

Haven't tried voice with AI Actions yet? See
[nine ways to be more productive with AI Actions](./2024-09-20-nine-ways-to-be-more-productive-with-ai-actions.md).
