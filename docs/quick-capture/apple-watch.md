---
title: Apple Watch app
description: Record a voice memo from your wrist, even offline, and have it sync to your phone.
sidebar_position: 6
---

# Apple Watch app

Capture thoughts wherever inspiration strikes. With the Apple Watch app you can:

- Record memos with a single tap using the Carbon Voice complication.
- Record even when offline.
- Sync automatically with the mobile app.

Make sure you're on the latest version to use it.

📺 [Watch: Carbon Voice on Apple Watch](https://youtu.be/urx2uBXdyLI)

## Related

- [iPhone home screen widgets](iphone-widgets.md)
- [Voice Memos](../voice-memos/index.md)
